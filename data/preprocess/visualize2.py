...
def read_h36m(args):
    ...
    data = read_pkl('../motion3d/H36M-27/test/%08d.pkl' % args.sequence_number)
    input_data = data['data_input']
    sample_joint_seq = data['data_label']
    input_data = input_data.transpose(1, 0, 2)
    sample_joint_seq = sample_joint_seq.transpose(1, 0, 2)
    input_data = (input_data / scale_factor) @ cam2real
    sample_joint_seq = (sample_joint_seq / scale_factor) @ cam2real
    return sample_joint_seq, input_data
...
def read_mpi(args):
    ...
    sequence_2d, sequence_3d = dataset[args.sequence_number]
    sequence_3d = sequence_3d.cpu().numpy()
    sequence_2d = sequence_2d.cpu().numpy()
    ...
    sequence_2d = sequence_2d.transpose(1, 0, 2)
    sequence_2d[14, ...] = 0
    sequence_2d = sequence_2d @ cam2real
    
    convert_h36m_to_mpi_connection()
    return sequence_3d, sequence_2d
...
def main():
    ...
    def update(frame):
        ...
        ax.scatter(x.mean(), y.mean(), z.mean(), c='y')
        
        x = input_data[:, frame, 0]
        y = np.zeros_like(input_data[:, frame, 1])
        z = input_data[:, frame, 2]

        for connection in connections:
            start = input_data[connection[0], frame, :]
            end = input_data[connection[1], frame, :]
            xs = [start[0], end[0]]
            ys = [start[1], end[1]]
            zs = [start[2], end[2]]
        ...
        ax.scatter(x.mean(), 0, z.mean(), c='g')
    ...
    sample_joint_seq, input_data = dataset_reader_mapper[args.dataset](args)
    
    input_data[:, :, 1] = 0
    
    sample_joint_seq[:, :, 1] = 0
